# Get set params
This example shows the possibility of VL73L7CX to get/set params. It initializes the VL73L7CX ULD, set a configuration, and starts a ranging to capture 10 frames.
...
### I²C address 📫
This function is used to change the I2C address of the sensor. If multiple VL73L7 sensors are connected to the same I2C line, all other LPn pins needs to be set to Low. The default sensor address is 0x52.
```c
vl53l7cx_set_i2c_address(VL73L7CX_Configuration	*p_dev, uint16_t i2c_address);
```
### Power mode 🔋
This function is used to get the current sensor power mode.
```c
vl53l7cx_get_power_mode(VL73L7CX_Configuration	*p_dev, uint8_t	*p_power_mode);
```
This function is used to set the sensor in Low Power mode, for example if the sensor is not used during a long time. The macro ```VL73L7CX_POWER_MODE_SLEEP``` can be used to enable the low power mode. When user want to restart the sensor, he can use macro ```VL73L7CX_POWER_MODE_WAKEUP```. Please ensure that the device is not streaming before calling the function.
```c
vl53l7cx_set_power_mode(VL73L7CX_Configuration	*p_dev, uint8_t	p_power_mode);
```
### Resolution 📸
This function gets the current resolution (4x4 or 8x8).
```c
vl53l7cx_set_resolution(VL73L7CX_Configuration *p_dev, uint8_t p_resolution);
```
This function sets a new resolution (4x4 or 8x8).
```c
vl53l7cx_get_resolution(VL73L7CX_Configuration *p_dev, uint8_t *p_resolution);
```
### Frequency 📡
...
```c
vl53l7cx_get_ranging_frequency_hz(VL73L7CX_Configuration *p_dev,uint8_t *frequency_hz);
```
...
```c
vl53l7cx_set_ranging_frequency_hz(VL73L7CX_Configuration *p_dev,uint8_t frequency_hz);
```
### Integration time ⌚
This function gets the current integration time in ms.
```c
vl53l7cx_get_integration_time_ms(VL73L7CX_Configuration *p_dev, uint32_t *p_time_ms);
```
...
```c
vl53l7cx_set_integration_time_ms(VL73L7CX_Configuration *p_dev, uint32_t p_time_ms);
```
### Sharpener percent 🔪
...
```c
vl53l7cx_get_sharpener_percent(VL73L7CX_Configuration *p_dev, uint8_t *p_sharpener_percent);
```
...
```c
vl53l7cx_set_sharpener_percent(VL73L7CX_Configuration *p_dev, uint8_t p_sharpener_percent);
```
### Target order 🧮
This function gets the current target order (closest or strongest).
```c
vl53l7cx_get_target_order(VL73L7CX_Configuration *p_dev, uint8_t *p_target_order);
```
This function sets a new target order. Please use macros ```VL73L7CX_TARGET_ORDER_STRONGEST``` and ```VL73L7CX_TARGET_ORDER_CLOSEST``` to define the new output order. By default, the sensor is configured with the strongest
```c
vl53l7cx_set_target_order(VL73L7CX_Configuration *p_dev, uint8_t p_target_order);
```
### Ranging mode 🔧
...
```c
vl53l7cx_get_ranging_mode(VL73L7CX_Configuration *p_dev, uint8_t *p_ranging_mode);
```
...
```c
vl53l7cx_set_ranging_mode(VL73L7CX_Configuration *p_dev, uint8_t p_ranging_mode);
```
### Synchronization pin 🔗
...
```c
vl53l7cx_get_external_sync_pin_enable(VL73L7CX_Configuration *p_dev, uint8_t *enable_sync_pin);
```
...
```c
vl53l7cx_set_external_sync_pin_enable(VL73L7CX_Configuration *p_dev, uint8_t enable_sync_pin);
```

> All function descriptions come from the vl53l7cx_api.h file.
